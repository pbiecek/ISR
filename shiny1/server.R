...
function(input, output) {
  ...
  output$ggPlotDiffNo <- renderPlot({
    di <- dataInput()
    theGGPlot(di, whiskers = TRUE)
  })
  
  output$ggPlotDiffNo2 <- renderPlot({
    di <- dataInput()
    theGGPlot(di, whiskers = FALSE)
  })
  
  output$ggPlotDiffInit <- renderPlot({
    di <- dataInput()
    theGGPlot(di, whiskers = TRUE, onX="Initial_Value")
  })
  
  output$ggPlotDiffInit2 <- renderPlot({
    di <- dataInput()
    theGGPlot(di, whiskers = FALSE, onX="Initial_Value")
  })
  ...
  output$ggPlotDots <- renderPlot({
    di <- dataInput()
    if (is.null(di)) return(NULL)

    frac <- mean(abs(di$diff)<=20)
    
    maxx <- max(c(di$Initial_Value, di$Repeat_Value), na.rm=TRUE)
    
    pl <- ggplot(di, aes(Initial_Value, Repeat_Value, color=absDiff)) +
      geom_abline(slope=1, linetype=2, color="grey") 
    if (any(abs(di$diff) > 20)) 
      pl <- pl + 
      geom_linerange(data=di[abs(di$diff) > 20,], 
                     aes(x=Initial_Value, ymin=Initial_Value, ymax=Repeat_Value), color="red3")
    pl <- pl +
      geom_point() +
      geom_abline(slope=0.8, linetype=2) + 
      geom_abline(slope=1.2, linetype=2) + 
      theme_bw() +
      scale_y_continuous(limits=c(0,maxx*1.05), expand = c(0,0)) +
      scale_x_continuous(limits=c(0,maxx*1.05), expand = c(0,0)) +
      scale_color_manual(values=c("black","red3"))+
      theme(legend.position="none", text=element_text(size=15))
    pl
  })
  
  output$ggPlotDots2 <- renderPlot({
    di <- dataInput()
    if (is.null(di)) return(NULL)
    
    frac <- mean(abs(di$diff)<=20)
    
    maxD <- max(abs(c(di$Initial_Value - di$Repeat_Value)), na.rm=TRUE)
    maxI <- max(di$Initial_Value, na.rm=TRUE)
    
    pl <- ggplot(di, aes(Initial_Value, Repeat_Value-Initial_Value, color=absDiff)) +
      geom_abline(slope=0, intercept=0, linetype=2, color="grey") 
    if (any(abs(di$diff) > 20)) 
      pl <- pl + 
      geom_linerange(data=di[abs(di$diff) > 20,], 
                     aes(x=Initial_Value, ymin=0, ymax=Repeat_Value-Initial_Value), color="red3")
    pl <- pl +
      geom_point() +
      theme_bw() +
      geom_abline(slope=-0.2, linetype=2) + 
      geom_abline(slope=0.2, linetype=2) + 
      scale_y_continuous(limits=maxD*c(-1.05,1.05), expand = c(0,0)) +
      scale_x_continuous(limits=c(0,maxI*1.05), expand = c(0,0)) +
      scale_color_manual(values=c("black","red3"))+
      theme(legend.position="none", text=element_text(size=15))
    pl
  })
